...
public interface KeyedDispatcherTests {
    
    @Test
    default void keyedDispatcher_BeforeOpen(@Mock Runnable runnable) {
        withMetalog(b -> {}, (contracts, metalog) -> {
            final Dispatcher dispatcher = contracts.claim(Dispatcher.KEYED_FACTORY).get();
