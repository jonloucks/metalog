package io.github.jonloucks.metalog.smoke;
...
public final class Main {
    ...
    private static void validateHelper(boolean useReflection, boolean useServiceLoader) {
        ...
        withContracts(contractsConfig, contracts -> {
            ...
            withMetalog(metalogConfig, metalog ->
                Stub.validate(contracts, metalog));
        });
    }
    ...
    @SuppressWarnings({"try"})
    private static void withMetalog(Metalog.Config config, Consumer<Metalog> consumerBlock) {
        final Metalog.Config validConfig = configCheck(config);
        final Consumer<Metalog> validConsumerBlock = nullCheck(consumerBlock, "Block must be present.");
        final Metalog contracts = GlobalMetalog.createMetalog(validConfig);
